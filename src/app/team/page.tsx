...
export default function TeamPage() {
  ...
  const handleInvite = async () => {
    ...
    setSending(true)
    setError(null)
    const { error } = await supabase.from('team_invites').insert({ owner_id: workspaceId, email: email.trim().toLowerCase() })
      ...
      <main className="flex-1 overflow-y-auto scrollbar-thin p-6 pl-20 md:pl-6">
        ...
        {user?.role === 'owner' ? (
          <div className="rounded-lg border p-4 mb-6 max-w-md" style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}>
            ...
            {error && <p className="text-xs mt-2" style={{ color: '#9B1C1C' }}>{error}</p>}
            <p className="text-xs mt-2" style={{ color: 'var(--text-muted)' }}>They'll get full access the moment they sign in with this email — no action needed from them beforehand.</p>
          </div>
        ) : (
        ...
        <div className="max-w-md">
          ...
          {members.length === 0 ? (
            ...
            <div className="flex flex-col gap-2 mb-6">
              {members.map(m => (
                <div key={m.id} className="flex items-center justify-between px-4 py-3 rounded-lg border" style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}>
                  <div>
                    <p className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>{m.full_name || m.email}</p>
                    <p className="text-xs" style={{ color: 'var(--text-muted)' }}>{m.email}</p>
                  </div>
                  {user?.role === 'owner' && (
                    <button onClick={() => removeMember(m.id)} className="text-xs" style={{ color: '#9B1C1C' }}>Remove</button>
                  )}
                </div>
              ))}
            </div>
          ...
          {invites.length > 0 && (
            <>
              ...
              <div className="flex flex-col gap-2">
                {invites.map(inv => (
                  <div key={inv.id} className="flex items-center justify-between px-4 py-3 rounded-lg border" style={{ background: 'var(--warm-white)', borderColor: 'var(--border)' }}>
                    <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>{inv.email}</p>
                    {user?.role === 'owner' && (
                      <button onClick={() => revokeInvite(inv.id)} className="text-xs" style={{ color: 'var(--text-muted)' }}>Cancel</button>
                    )}
                  </div>
                ))}
              </div>
